...
public class PrimaryKeyFactory {
	 ...
	 * 生成主键， 1秒最多1000个
	 */
	public static synchronized String generatePK(String head){
		if(SERIALNUMBER == 9) {
			SERIALNUMBER = 1;
		}
	 ...
	 * 生成主键， 1秒最多1000个
	 */
	public static synchronized long generatePK(){
	...
	public static void main(String[] args) throws Exception{
		...
		System.out.println(PrimaryKeyFactory.class);
		/*//多线程同时生成查看是否有重复问题
		...
		for(int i=0;i<length;i++){
			arr[i] = new Thread(new Runnable(){
				public void run(){
					...
					for(int j=0; j<20; j++){
						String num = ""+PrimaryKeyFactory.generatePK();
